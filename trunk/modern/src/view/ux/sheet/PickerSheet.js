 ...
 *     picker.show();
 */
Ext.define('UX.sheet.PickerSheet', {
    extend: 'Ext.Sheet',
    ...
    initialize: function () {
        console.log('time picker initialize');
        var me = this,
     ...
     * @private
     */
    applyBottombar: function (config) {
        console.log('applyBottomBar');
        if (config === true) {
            config = {};
        }
        ...
        return Ext.factory(config, 'Ext.TitleBar', this.getBottombar());
    },

    /**
     * @private
     */
    updateBottombar: function (newBottombar, oldBottombar) {
        if (newBottombar) {
            this.add(newBottombar);
        }

        if (oldBottombar) {
            this.remove(oldBottombar);
        }
    },
    ...
    updateSlots: function (newSlots) {
        ...
        if (innerItems.length > 0) {
            ...
            //日期控件，左边显示日期的样式默认为"-webkit-box-flex: 3; flex: 3 3 0px;"，宽度不够，设置长一点
            //innerItems[0].el.dom.style="-webkit-box-flex: 4; flex: 4 4 0px;";
        }
