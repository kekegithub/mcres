 ...
 * 模块说明：新建工程报修单
 * 修改历史：
 * 2017/4/7 - zhaolingling - 创建
 */
Ext.define('UX.select.SelectField', {
        ...
        if (!phonePicker) {
            ...
            me.phonePicker = phonePicker = Ext.create('Ext.picker.Picker', Ext.apply({
                slots: [{
                    ...
                    store: me.getStore(),
                    listeners: {
                        itemtap: me.slotItemTap,
                        scope: me
                    }
                }],
                cls: 'hdselectpicker',   //加这个是因为日期控件也是用picker样式的，但是查询条件过滤器下拉选择和日期选择不太一样，用以区分设置样式
                doneButton: false,
                cancelButton: false,
                scrollable: false
            }, config));
    ...
    slotItemTap: function (slot, index, target, record, e, eOpts) {

        var me = this;
        newValue = record.data.value,
            store = me.getStore(),
            index = store.find(me.getValueField(), newValue, null, null, null, true),
            record = store.getAt(index);

        me.setValue(record);
        var icon_field = document.getElementById(me.id).getElementsByClassName('x-font-icon')[0].classList;
        icon_field.remove('selectfield-selected');
        var text_field = document.getElementById(me.id).getElementsByClassName('x-input-body-el')[0];
        text_field.style.background = "#fff";
        var el = document.getElementsByClassName('x-mask');
        for (var i = 0; i < el.length; i++) {
         el[i].classList.remove("x-hidden-display");
        }
        me.phonePicker.hide();
    },
    ...
    clsIndex: function (list, clsname) {
        var index = -1;
     ...
     */
    showPicker: function () {
        var el = document.getElementsByClassName('x-picker');
        console.dir(el);
        for (var i = 0; i < el.length; i++) {
            el[i].hidden = true;
        }

        el = document.getElementsByClassName('x-mask');
        for (var i = 0; i < el.length; i++) {
            el[i].classList.remove("x-hidden-display");
        }

        el = document.getElementsByClassName('x-font-icon');
        for (var i = 0; i < el.length; i++) {
            el[i].classList.remove("selectfield-selected");
        }

        el = document.getElementsByClassName('x-input-body-el');
        for (var i = 0; i < el.length; i++) {
        ...
        var icon_field = document.getElementById(me.id).getElementsByClassName('x-font-icon')[0].classList;
        icon_field.add("selectfield-selected");
        var text_field = document.getElementById(me.id).getElementsByClassName('x-input-body-el')[0];
        text_field.style.background = "#ffffff";
