 ...
 * 项目名：hdts
 * 文件名：DateTimePickerField
 * 模块说明：月日-时-分，日期Picker
 *         参考源码Ext.field.DatePicker基础上修改
 * 修改历史：
 * 2017/3/20 - zhaolingling - 创建
 */
Ext.define('UX.date.DateTimePickerField', {
    extend: 'Ext.field.Picker',
    alternateClassName: 'Ext.form.DateTimePickerField',
    xtype: 'datetimepickerfield',

    requires: [
        //'Ext.field.trigger.Date',
        'UX.date.DateTimeSelect'
    ],
    ...
    getPicker: function() {
        ...
        if (picker && !picker.isPicker) {
            picker = Ext.factory(picker, UX.date.DateTimeSelect);
            if (value !== null) {
